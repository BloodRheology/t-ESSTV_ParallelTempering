...
qoptim = ParTemp(objective, ... % function to be minimized
                ...
                1, ...          % EBHot (unity here)
                [SSqoptim*0.7,0,0,0],'MIN', ... % Minimum value constraint
                [SSqoptim*1.3,start(1,8:10)*1.5],'MAX'); % Maximum value constraint

np.save("CALCULATIONS\tESSTV_partemp.mat","qoptim")
np.save("DATA\tESSTV_partemp.mat","exp")

%% Plotting
calc = np.load("CALCULATIONS\tESSTV_partemp.mat");
DATA = np.load("DATA\tESSTV_partemp.mat");
